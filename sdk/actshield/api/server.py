...
Run: python -m actshield.api.server

Endpoints mirror the frontend service/api.ts contract.
All data comes from the live ActShield SDK runtime.
"""
...
# ── ActShield SDK imports ────────────────────────────────────────────────────
from actshield.agents.identity import AgentIdentity, AgentTrustLevel
...
from actshield.tracing.tracer import CausalGraph, CausalNode, CausalEdge
...
# ── App ───────────────────────────────────────────────────────────────────────
app = FastAPI(
    title="ActShield Dashboard API",
    description="Live security data from the ActShield SDK runtime",
    version="1.0.0",
)
...
# Build a live ActShield runtime with demo agents
_config = ActShieldConfig(enforcement_mode="STRICT")
...
guard = ActShield(config=_config)
...
def evaluate_policy(req: PolicyEvalRequest):
    ...
    g = ActShield(agent=agent, config=_config)
...
def list_policies():
    ...
    try:
        from actshield.policy.policy import PolicyEvaluator
...
from actshield.posture import PostureEngine, SecurityPostureSnapshot, PostureDiffEngine
from actshield.incidents import IncidentEngine, IncidentState, SecurityIncident
from actshield.drift import BehavioralBaselineTracker, DriftCategory, DriftSeverity
from actshield.gates import SecurityGateEvaluator, SecurityGateResult
