...
class ActShieldShell:
    """Interactive command-line shell and security REPL for actshield."""

    def __init__(self, guard: Optional[ActShield] = None) -> None:
        self.console = Console()
        self.guard = guard or ActShield(mode="strict")
        self.guard.start()
        ...
        while True:
            try:
                ...
                if cmd in ("exit", "quit", "q"):
                    self.console.print("[dim magenta]Exiting ActShield console. Security runtime active.[/dim magenta]")
                    break
    ...
    def cmd_help(self) -> None:
        table = Table(
            title="[bold magenta]ActShield SECURITY COMMANDS[/bold magenta]",
        ...
        commands = [
            ...
            ("dashboard [start|stop|serve]", "Control the embedded ActShield dashboard service"),
    ...
    def cmd_status(self) -> None:
        table = Table(title="[bold magenta]ActShield COMMAND CENTER[/bold magenta]", border_style="magenta")
    ...
    def cmd_dashboard(self, args: List[str]) -> None:
        self.console.print(
            Panel(
                "ActShield Dashboard service running at: [bold bright_cyan]http://localhost:3000[/bold bright_cyan]\n"
                "Backend API runtime available at: [bold bright_cyan]http://localhost:8000/docs[/bold bright_cyan]",
                title="[bold magenta]ActShield DASHBOARD CONTROL[/bold magenta]",
                border_style="magenta",
            )
        )
    ...
    def cmd_init(self) -> None:
        self.console.print(
            Panel(
                "[bold green]✓ Initialized actshield.yaml security configuration.[/bold green]\n"
