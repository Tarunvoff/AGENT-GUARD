"""ActShield End-to-End Multi-Agent Security Simulation Demo.
...
def run_demo(guard: Optional[ActShield] = None) -> None:
    """Run interactive security simulation."""
    console = Console()
    guard = guard or ActShield(mode="strict")
    guard.start()

    console.print(
        Panel.fit(
            "[bold magenta]🛡️  ActShield LIVE SECURITY SIMULATION[/bold magenta]\n"
    ...
    console.print(
        Panel.fit(
            "[bold green]✓ ActShield SECURITY STORY VALIDATED SUCCESSFULLY[/bold green]\n"
