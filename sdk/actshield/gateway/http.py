...
class HTTPInterceptedResponse:
    """Standardized HTTP response wrapped with ActShield security provenance."""
    status_code: int
    ...
    def request(
        ...
        if decision.is_blocked or apiris_analysis.recommended_action == "BLOCK":
            ...
            return HTTPInterceptedResponse(
                status_code=403,
                body={"error": "ActShield HTTP Interception: Access Blocked", "reason": reason},
                allowed=False,
        ...
        if self._mock_transport:
            ...
            status_code = 200
            res_headers = {"Content-Type": "application/json", "X-ActShield-Gateway": "Verified"}
            res_body = {
                "status": "SUCCESS",
                "method": method_upper,
                "url": url,
