...
from agentguard.tracing.events import SecurityEvent
...
class SIEMExporter:
    ...
    def format_event_cef(cls, event: SecurityEvent) -> str:
        ...
        return f"CEF:0|AgentGuard|AgentGuard-SDK|0.3.0|{event.event_type.value}|{event.event_type.value}|{severity}|{ext_str}"
    ...
    def format_event_syslog(cls, event: SecurityEvent) -> str:
        ...
        return f"<134>1 {ts} agentguard-runtime security-audit - {event.event_id} [agentguard@44400 trace_id=\"{event.trace_id}\" event_type=\"{event.event_type.value}\"] {payload_str}"
